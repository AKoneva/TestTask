...
var favorites : [FavoriteArticles]?
 ...
 // MARK: - getMostEmailed
func getMostEmailed(){
    AF.request("https://api.nytimes.com/svc/mostpopular/v2/emailed/30.json?api-key=1P0w450CnIPDkC0wapET8prY75GyrrRl").responseJSON { response in
...
// MARK: - getMostShared
func getMostShared(){
      AF.request("https://api.nytimes.com/svc/mostpopular/v2/shared/30.json?api-key=1P0w450CnIPDkC0wapET8prY75GyrrRl").responseJSON { response in
        let json = response.data!
        switch response.result {
         case .success:
             do {
...
// MARK: - getMostViewed()
func getMostViewed(){
   let request = AF.request("https://api.nytimes.com/svc/mostpopular/v2/viewed/30.json?api-key=1P0w450CnIPDkC0wapET8prY75GyrrRl").validate()
